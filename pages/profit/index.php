<?
  ...
  $PAGE_ID = 4; // Identifier of this page
...
<head>
  ...
  <title>
    <?= $CONTENT__DATA['GET_WIN'].' | '.$CONTENT__DATA__ALL_PAGE['PAGE_TITLE']; ?>
  </title>
</head>
<body data-lang-code="<?= $_SESSION['LANG'];?>">
	<div class="popup__message" id="error__input__data">
  	<p><?= $CONTENT__DATA['ERROR_DATA']; ?></p>
  	<button><?= $CONTENT__DATA__ALL_PAGE['CLOSE']; ?></button>
  </div>

  <div class="popup__message" id="error__send__mail">
  	<p>
  		<?= $CONTENT__DATA['UNEXPECTED_ERROR']; ?>
  	</p>
  	<button><?= $CONTENT__DATA__ALL_PAGE['CLOSE']; ?></button>
  </div>

  <div class="popup__message" id="pay__ok">
  	<p><?= $CONTENT__DATA['MESSAGE_SENT']; ?></p>
  	<button><?= $CONTENT__DATA__ALL_PAGE['CLOSE']; ?></button>
  </div>

  <div class="popup__message" id="pay__progress">
  	<p><?= $CONTENT__DATA['REQUEST_PROCESSING']; ?></p>
  	<button><?= $CONTENT__DATA__ALL_PAGE['CLOSE']; ?></button>
  </div>

  <div class="popup__message" id="pay__completed">
  	<p><?= $CONTENT__DATA['REWARDS_PAID']; ?></p>
  	<button><?= $CONTENT__DATA__ALL_PAGE['CLOSE']; ?></button>
  </div>

  <div class="popup__message" id="not__winner">
  	<p><?= $CONTENT__DATA['NOT_WINNER']; ?></p>
  	<button><?= $CONTENT__DATA__ALL_PAGE['CLOSE']; ?></button>
  </div>
	<?
		require_once SITE__DIR.'template/header.php';
		require_once SITE__DIR.'template/profit.php';
		require_once SITE__DIR.'template/footer.php';
	?>
	<script async src="/js/profit.min.js"></script>
</body>
</html>
